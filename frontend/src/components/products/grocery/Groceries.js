...
const RenderGrocery = ({grocery,deleteProduct}) =>{
    ...
    const handleClick = (grocery) =>{ 
        history.push({ 
            pathname: `/products/details/${grocery.groceryName}`,
            state: {data: grocery}
        });
    }
    ...
    const handleDelete = async(id) =>{
        ...
        await deleteProduct(id,grocery.category)
        await toggleModal();
    ...
    return(
        <div className="p-0 m-2">
            <Card
                ...
                style={{height:'425px'}}  
            >
                    <CardImg className="img-q" width="100" height="250" top src={baseUrl+grocery.img} alt={grocery.groceryName} />
                    ...
                    <CardBody className="text-center"> 
                         <p><b>{grocery.groceryName}</b></p>  
                            ...
                            <h5 className="mt-1"><b><i className="fa fa-inr"></i>{grocery.price}.0</b></h5> 
                    </CardBody>
            </Card> 
            <Modal
                isOpen={isModalOpen}
...
function Groceries(props) {
    ...
    const groceries = props.groceries?.slice(paginaton.start,paginaton.end).map((grocery)=>{
        return ( 
            <div className="col-6 col-sm-3 m-0 p-0"  key={grocery._id}>
                <RenderGrocery grocery={grocery} deleteProduct = {props.deleteProduct} />
            </div>
        )
    })
    if(props.isLoading){
        ...
        return ( 
            <div className="row">
                ...
                <div className="d-flex justify-content-end align-items-end">
                    <Pagination  
                        count={Math.ceil(props.groceries?.length/showPerPage)}
