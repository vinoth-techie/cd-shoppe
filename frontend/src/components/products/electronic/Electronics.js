...
const RenderElectronics = ({electronic,deleteProduct}) =>{
    ...
    const handleClick = (electronic) =>{
        console.log(electronic);
        history.push({ 
            pathname: `/products/details/${electronic.electronicName}`,
    ...
    const handleDelete = async(id) =>{
        ...
        await deleteProduct(id,electronic.category)
        await toggleModal();
    ...
    return(
        <div className="p-0 m-2">
            <Card
                ...
                style={{height:'425px'}}  
            >
                    <CardImg className="img-q" width="100" height="250" top src={baseUrl+electronic.img} alt={electronic.electronicName} />
                    ...
                    <CardBody className="text-center"> 
                         <p><b>{electronic.electronicName}</b></p>  
                            <Rating  
                                className="mt-0"
                                name="simple-controlled" 
                                value={electronic.rating}
                                readOnly
                                style={{fontSize:'1.3rem'}}
                            />&nbsp; | <i className="mt-1" style={{fontSize:'13px',marginTop:'-10px'}}> 7 &nbsp; reviews</i>
                           
                            <h5 className="mt-1"><b><i className="fa fa-inr"></i>{electronic.price}.0</b></h5> 
                    </CardBody>
            </Card> 
            <Modal
                isOpen={isModalOpen}
                ...
                <ModalBody className="row p-4">
                    ...
                    <div className="col-12 text-center"> 
                        ...
                        <Button
                            onClick={()=>handleDelete(electronic._id)}
                            variant="contained"
                            color="secondary"
                        >
                            Yes
...
function Electronics(props) { 
    ...
    const electronics = props.electronics?.slice(paginaton.start,paginaton.end).map((electronic)=>{
        return ( 
            <div className="col-6 col-md-3 m-0 p-0"  key={electronic._id}>
                <RenderElectronics electronic={electronic}  deleteProduct = {props.deleteProduct} />
            </div>
        )
    });
    if(props.isLoading){
        ...
        return ( 
            <div className="row">
                ...
                <div className="d-flex justify-content-end align-items-end">
                        <Pagination  
                            count={Math.ceil(props.electronics?.length/showPerPage)}
